...
import { Sheet, SheetContent, SheetDescription, SheetHeader, SheetTitle } from '@/registry/bases/base/ui/sheet';
...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      const derivedOpen = typeof value === 'function' ? value(open) : value;
      if (setOpenProp) {
        setOpenProp(derivedOpen);
      } else {
        setOpenState(derivedOpen);
      }

      // This sets the cookie to keep the sidebar state.
      document.cookie = `${SIDEBAR_COOKIE_NAME}=${derivedOpen}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}`;
    },
    [setOpenProp, open],
  );
