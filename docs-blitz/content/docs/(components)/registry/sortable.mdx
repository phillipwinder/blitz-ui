...
```bash
npx shadcn@latest add @blitz-ui/r-sortable
```
...
<ComponentSource styleName="base-nove" name="sortable" title="components/blitz-ui/sortable.tsx" />
...
```tsx
import { Sortable, SortableItem, SortableItemHandle } from "@/components/blitz-ui/r-sortable"
```
