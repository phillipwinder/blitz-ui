...
```bash
npx shadcn@latest add @blitz-ui/date-selector
```
...
<ComponentSource name="date-selector" title="components/blitz-ui/date-selector.tsx" />
...
```tsx
import { DateSelector, type DateSelectorValue } from "@blitz-ui/react/date-selector"
```
