...
function collectPages(nodes: PageTreeNode[]): { url: string; name: React.ReactNode }[] {
        ...
        >
          ...
          <CommandList className="no-scrollbar min-h-80 scroll-pt-2 scroll-pb-1.5">
            ...
            {navItems && navItems.length > 0 && (
              ...
              >
                {navItems.map((item) => (
                  ...
                  >
                    <ArrowRight />
                    <span className={cn(item.soon && 'opacity-50')}>{item.label}</span>
                    {item.soon && (
                ...
                >
                  {pages.map((item) => {
                    ...
                    return (
                      ...
                      >
                        {isComponent ? <SquareDashedIcon /> : <ArrowRight />}
                        {item.name}
                      </CommandMenuItem>
                    );
                  })}
            ...
            {patternCategories.length > 0 && (
              <CommandGroup heading="Patterns" className="p-0! **:[[cmdk-group-heading]]:p-3!">
                {patternCategories.map((category) => (
                  <CommandMenuItem
                    key={`pattern-cat-${category.name}`}
                    value={`Pattern Category ${category.label}`}
                    onHighlight={handleCategoryHighlight}
                    keywords={['pattern', 'category', category.name, category.label]}
                    onSelect={() => {
                      runCommand(() => router.push(`/patterns/${normalizeSlug(category.name)}`));
                    }}
                  >
                    <LayoutGridIcon />
                    {category.label}
